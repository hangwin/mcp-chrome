...
└─ postinstall.js
   ...
   └─ 如果失败 ➜ 提示用户运行 agent-chrome-mcp register --system
      └─ 需要手动使用管理员权限运行
```
...
```bash
# macOS/Linux
sudo agent-chrome-mcp register --system

# Windows (以管理员身份运行命令提示符)
agent-chrome-mcp register --system
```
...
```
sudo agent-chrome-mcp register
```
...
```
注册流程
├─ 用户级别注册 (agent-chrome-mcp register)
│  ├─ 获取用户级别清单路径
│  ├─ 创建用户目录
│  ├─ 生成清单内容
│  ├─ 写入清单文件
│  └─ Windows平台：创建用户级注册表项
│
└─ 系统级别注册 (agent-chrome-mcp register --system)
   ├─ 检查是否有管理员权限
   │  ├─ 有权限 → 直接创建系统目录和写入清单
   │  └─ 无权限 → 提示用户使用管理员权限运行
   └─ Windows平台：创建系统级注册表项
...
3. 如果没有管理员权限：
   - 提示用户使用管理员权限重新运行命令
   - macOS/Linux: `sudo agent-chrome-mcp register --system`
   - Windows: 以管理员身份运行命令提示符
...
│  │  └─ 运行 agent-chrome-mcp fix-permissions
│  │
│  └─ 尝试 agent-chrome-mcp register --system
│
├─ 路径问题
│  ├─ 检查Node.js安装 (node -v)
...
3. **修复执行权限问题**
   ...
   **解决方案**：
   ...
   ```bash
   agent-chrome-mcp fix-permissions
   ```
   ...
   ```bash
   # 查找安装路径
   npm list -g agent-chrome-mcp
   # 或者对于 pnpm
   pnpm list -g agent-chrome-mcp

   # 设置执行权限（替换为实际路径）
   chmod +x /path/to/node_modules/agent-chrome-mcp/run_host.sh
   chmod +x /path/to/node_modules/agent-chrome-mcp/index.js
   chmod +x /path/to/node_modules/agent-chrome-mcp/cli.js
   ```
   ...
   **解决方案**：
   ...
   ```bash
   # 卸载
   npm uninstall -g agent-chrome-mcp
   # 或 pnpm uninstall -g agent-chrome-mcp

   # 重新安装
   npm install -g agent-chrome-mcp
   # 或 pnpm install -g agent-chrome-mcp
...
5. 尝试使用系统级别安装
   - 使用 `agent-chrome-mcp register --system` 命令
   - 或直接使用管理员权限运行
